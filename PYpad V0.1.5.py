...
def changed(event=None):
    ...
    if text_editor.edit_modified():###checks if any character is added or not
        ...
        status_bar.config(text=f' Words: {words} Characters : {characters}'' |V0.1.5 Created by SuperPythonGuy')
...
def shortcut():
    messagebox.showwarning("WARNING","THIS IS VERY EXPERIMENTAL AND COULD CATATROPICHLY FAIL")
    
edit.add_command(label='Paste',image=paste_icon,compound=tk.LEFT, accelerator='Ctrl+V',command=lambda:text_editor.event_generate("<Control v>"))
edit.add_command(label='Cut',image=cut_icon,compound=tk.LEFT, accelerator='Ctrl+X',command=lambda:text_editor.event_generate("<Control x>"))
edit.add_command(label='Clear All',image=clear_all_icon,compound=tk.LEFT, accelerator='Ctrl+ALt+X',command=lambda:text_editor.delete(1.0,tk.END))
edit.add_command(label='Find',image=find_icon,compound=tk.LEFT, accelerator='Ctrl+F',command=find_func)
edit.add_command(label='About',compound=tk.LEFT,command=messagebox.showinfo('PYpad','Created by SuperPythonGuy V0.1.5 Under Development'))
view.add_command(label='Shortcuts',command=shortcut())
...
main_application.bind("<Control-w>",new_file)
